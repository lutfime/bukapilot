...
def prune_orphan_thumb_cache() -> None:
  ...
  for entry in thumb_dir.iterdir():
    ...
    if entry.name.endswith(".tmp.jpg"):
      continue
    parsed = _parse_thumb_cache_name(entry.name)
    if not parsed:
      continue
...
def _generate_thumb(hevc_path: str, cache_path: Path) -> bool:
  ...
  try:
    ...
    if proc.returncode != 0:
      ...
      cloudlog.warning(f"thumbnail generation failed: {cmd} exit {proc.returncode}: {err}")
      return False
...
def ensure_thumb(hevc_path: str, cache_path: Path) -> bool:
  if _is_cache_valid(cache_path, hevc_path):
    return True
  return _generate_thumb(hevc_path, cache_path)


class ThumbGenerateJob:
  def __init__(self, hevc_path: str, cache_path: Path):
  ...
  def _run(self):
    ok = ensure_thumb(self.hevc_path, self.cache_path)
    with self._lock:
      self._ok = ok
      self._done = True
