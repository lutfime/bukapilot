...
MIN_VEGO = 50.0 * 0.44704  # ~22.35 m/s (50 mph); only estimate at clean highway speeds
...
class LateralLagEstimator:
  ...
  def get_msg(self, valid: bool, debug: bool = False) -> capnp._DynamicStructBuilder:
    ...
    if liveDelay.status == log.LiveDelayData.Status.estimated:
      liveDelay.lateralDelay = float(np.clip(valid_mean_lag, MIN_LAG, MAX_LAG))
    else:
      liveDelay.lateralDelay = float(np.clip(self.initial_lag, MIN_LAG, MAX_LAG))
  ...
  def update_estimate(self):
    ...
    # pre-smooth to kill noise-induced false delay peaks
    desired_smoothed = masked_symmetric_moving_average(desired, okay, SMOOTH_K, SMOOTH_SIGMA)
    actual_smoothed = masked_symmetric_moving_average(actual, okay, SMOOTH_K, SMOOTH_SIGMA)
    valid_mask = okay & ~np.isnan(desired_smoothed) & ~np.isnan(actual_smoothed)

    # skip estimation when there's no lateral-accel signal
    if np.ptp(desired_smoothed[valid_mask]) < MIN_LAT_ACCEL_RANGE:
      return

    delay, corr, confidence = self.actuator_delay(desired_smoothed, actual_smoothed, valid_mask, self.dt, MIN_LAG, MAX_LAG)
